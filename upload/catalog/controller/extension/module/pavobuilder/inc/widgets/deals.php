...
public function render( $settings = array(), $content = '' ) {
	...
	foreach ($specials as $get_special) {
		if ( ! empty( $get_special['image'] ) ) {
                ...
                $src = empty( $settings['image_size'] ) || $settings['image_size'] == 'full' ? $server . 'image/' . $get_special['image'] : false;
                if ( $src === false && strpos( $settings['image_size'], 'x' ) ) {
                    $sizes = explode( 'x', $settings['image_size'] );
                    if ( ! empty( $sizes[0] ) && ! empty( $sizes[1] ) ) {
                        $src = $this->model_tool_image->resize( $get_special['image'], $sizes[0], $sizes[1] );
                    }
                }
	...
	if ( ! empty( $settings['banner_image'] ) ) {
		$settings['banner_size'] = strtolower( $settings['banner_size'] );
			...
			if ( $srcs === false && strpos( $settings['banner_size'], 'x' ) ) {
				$sizes = explode( 'x', $settings['banner_size'] );
				if ( ! empty( $sizes[0] ) && ! empty( $sizes[1] ) ) {
					$srcs = $this->model_tool_image->resize( $settings['banner_image'], $sizes[0], $sizes[1] );
			}
		}
