...
class PA_Widget_Latest_Blogs extends PA_Widgets {
    ...
    public function render( $settings = array(), $content = '' ) {
            ...
            if ( ! empty( $post['image'] ) ) {
                ...
                if ( $src === false && strpos( $settings['image_size'], 'x' ) ) {
                    ...
                    if ( ! empty( $sizes[0] ) && ! empty( $sizes[1] ) ) {
                        $src = $this->model_tool_image->resize( $post['image'], $sizes[0], $sizes[1] );
                    }
                }
        ...
        return $this->load->view( 'extension/module/pavobuilder/pa_latest_blogs/pa_latest_blogs', array( 'settings' => $settings, 'content' => $content ) );
    }
