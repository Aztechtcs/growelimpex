...
class PA_Widget_Interactive_Banner extends PA_Widgets {
    ...
    public function render( $settings = array(), $content = '' ) {
        ...
        if ( ! empty( $settings['banner_image'] ) ) {
            ...
            $src = empty( $settings['banner_size'] ) || $settings['banner_size'] == 'full' ? $server . 'image/' . $settings['banner_image'] : false;

            if ( $src === false && strpos( $settings['banner_size'], 'x' ) ) {
                $this->load->model( 'tool/image' );
                $sizes = explode( 'x', $settings['banner_size'] );
                if ( ! empty( $sizes[0] ) && ! empty( $sizes[1] ) ) {
                    $src = $this->model_tool_image->resize( $settings['banner_image'], $sizes[0], $sizes[1] );
                }
            }
        ...
        return $this->load->view( 'extension/module/pavobuilder/pa_interactive_banner/pa_interactive_banner', array( 'settings' => $settings, 'content' => $content ) );
    }
