...
class ControllerExtensionModulePavothemer extends PavoThemerController {
	...
	public function customize() {
		...
		if( isset($this->request->post['customize']) && is_array($this->request->post['customize']) ){
			...
			$creator = array( 'customize' => $customize, 'font' => $font );
	...
	public function updateCustomize() {
		...
		// if ( $skin ) {
			...
			if ( $clone && file_exists( $file ) ) {
				...
				$creator = array( 'customize' => $customize, 'font' => $fonts );
				if ( ! $skin ) {
	...
	public function liveinstallsample() {
			...
			if ( ! $sample_file || ! file_exists( $sample_file ) ) {
				...
				if ( isset( $this->session->data['pavo_mass_install'] ) && $this->session->data['pavo_mass_install'] ) {
					...
					try {
						...
						$this->model_extension_pavothemer_sample->installSQL( $query, $mapping_data );
					} catch ( Exception $e ) {
		...
		if ( ! isset( $results['error'] ) ) {
			...
			$results['text'] = $this->language->get( 'entry_downloading_images' ); // entry_import_success_text
	...
	public function importSQL() {
		...
		$query = $sampleHelper->getImportSQL( $folder );
	...
	public function importLayouts() {
		...
		$folder = ! empty( $this->session->data['pavo_importing_profile'] ) ? $this->session->data['pavo_importing_profile'] : '';
		$profile = $folder ? $sampleHelper->getProfile( $folder ) : array( 'layouts' => array(), 'themes' => array() );
		$status = $this->model_extension_pavothemer_sample->importLayouts( false, $profile );
		$response = array(
	...
	public function exportTables() {
		...
		// url
		// $this->session->data['pavo_backbup_images'] = $images;
		...
		if ( empty( $fields['item'] ) ) {
			...
			foreach ( $fields['item'] as $k => $item ) {
				...
				if ( ! empty( $item['item'] ) ) {
					...
					if ( isset( $item['font'] ) ) {
						$value = isset( $customize_options['font'][ $item['id'] ] ) ? $customize_options['font'][ $item['id'] ] : '';
					} else {
	...
	protected function _skincreator( $creator = array(), $skin = false, $preview = false ) {
		...
		foreach ( $fonts as $key => $font ) {
			if ( ! empty( $font['name'] ) ) {
				$output[] = '$'.trim($key).'			 : "' . $font['name'] . '", Arial, sans-serif !default;';
				$font_importer[] = 'https://fonts.googleapis.com/css?family=' . implode( '+', explode( ' ', $font['name'] ) ) . ( ! empty( $font['weight'] ) ? ':' . $font['weight'] : '' );
			}
		}
		...
		if(  $this->language->get( 'direction' ) == 'rtl' ){
			...
			$content = rtlcss::transform( $content );
		}
