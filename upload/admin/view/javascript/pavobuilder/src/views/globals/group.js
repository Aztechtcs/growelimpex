	...
	_renderGroup( group = {}, index = 0 ) {
		...
		if ( this.fields.length > 0 ) {
			...
			this.fields.map( ( field ) => {
				...
				this.$( '.group-content #' + field.id + '-' + index + ' .group-body' ).append( _.template( $( '#pa-' + field.type + '-form-field' ).html(), { variable: 'data' } )( { field: field, settings: settings } ) );
			} );
			...
			if ( Object.keys( this.languageFields ).length > 0 ) {
				...
				_.map( window.PA_PARAMS.languages_list, ( language, key ) => {
					...
					_.map( this.languageFields, ( field, key ) => {
						...
						field.name = name;
						this.$( '.group-content #' + field_ID + '-' + index + ' .group-body #language-' + index + language.language_id ).append( _.template( $( '#pa-' + field.type + '-form-field' ).html(), { variable: 'data' } )( { field: field, settings: settings } ) );
					} );
				} );
			}
			// init scripts
