...
export default class EditForm extends Backbone.View {
	...
	renderFields() {
		...
		if ( current_screen ) {
			...
			if ( this.fields.general !== undefined && this.fields.general.fields !== undefined ) {
				_.map( this.fields.general.fields, ( field ) => {
					let name = field.name;
			...
			if ( Object.keys( screen_settings ).length > 0 ) {
				settings = { ...screen_settings, ...globalSettings, ...settings };
			}
	...
	_renderFields( clonefields = {}, tab = '', cloneSettings = {} ) {
		...
		_.map( clonefields.fields, ( field, key ) => {
			...
			if ( cloneField.type !== 'group' ) {
				...
				this.$( '#nav-' + tab ).append( _.template( $( '#pa-' + cloneField.type + '-form-field' ).html(), { variable: 'data' } )( { field: cloneField, settings: cloneSettings, widget: this.data.get( 'widget' ) } ) );
			} else {
		...
		new Promise( ( resolve, reject ) => {
			...
			_.map( this.fields, ( group, name ) => {
				_.map( group.fields, ( field ) => {
					...
					} else if ( name !== 'background' && name !== 'style' ) {
